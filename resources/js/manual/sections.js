...
export const MANUAL_SECTIONS = [
    ...
    howTos: [
      ...
      },
      {
        ...
        steps: [
          ...
          'On phones, the bottom bar jumps to the four actions you use most.',
          'Open Help (book icon) in the top bar anytime for this manual.',
        ],
      },
    ],
    actions: [
      { name: 'Open menu / Close menu', where: 'Top bar (phone)', does: 'Shows or hides the sidebar navigation.' },
      ...
      { name: 'Cart', where: 'Top bar', does: 'Opens your borrow cart. Badge shows how many lines are waiting.' },
  ...
  {
    id: 'catalog',
    title: 'Browse tools & cart',
    icon: 'grid',
    audience: 'Everyone',
    summary: 'Find equipment, add lines to the cart, join a waitlist when nothing is free.',
    howTos: [
      {
        ...
        steps: [
          ...
          'Either tap Add any available unit, or Choose a specific unit and Add this unit to cart.',
          'Open Cart from the top bar.',
    ...
    actions: [
      ...
      { name: 'Cart (count)', where: 'Catalog pages', does: 'Jumps to the cart.', anyOf: ['borrow_items'] },
      { name: 'Add any available unit', where: 'Category', does: 'Adds a flexible “any free unit of this type” line to the cart.', anyOf: ['borrow_items'] },
      { name: 'Choose a specific unit / Hide units', where: 'Category', does: 'Expands or collapses the unit list.' },
      { name: 'Add this unit to cart', where: 'Unit row / peek / unit detail', does: 'Adds that exact asset to the cart (when loanable and available).', anyOf: ['borrow_items'] },
      { name: 'Join waitlist / Cancel', where: 'Waitlist modal', does: 'Creates or dismisses a waitlist entry.', anyOf: ['borrow_items'] },
      { name: 'Unit photo or name', where: 'Category', does: 'Opens the unit peek sheet (photos, specs, add to cart).' },
      { name: 'Edit in inventory', where: 'Unit detail', does: 'Opens admin item editor (inventory managers).', anyOf: ['manage_inventory'] },
      { name: 'Less / More quantity', where: 'Cart', does: 'Changes how many of that type/line you need.', anyOf: ['borrow_items'] },
      { name: 'Remove', where: 'Cart line', does: 'Deletes that line from the cart.', anyOf: ['borrow_items'] },
      { name: 'This exact unit / Any free unit', where: 'Cart line', does: 'Controls whether approval must allocate your chosen asset or any matching type.', anyOf: ['borrow_items'] },
      { name: 'Today only / Tomorrow, 1 day / Next week, 3 days', where: 'Cart dates', does: 'Fills need-from / return-by with common presets.', anyOf: ['borrow_items'] },
      { name: 'Urgency chips (Not urgent → Today)', where: 'Cart', does: 'Sets request priority for the depot.', anyOf: ['borrow_items'] },
      { name: 'Empty cart', where: 'Cart', does: 'Clears all lines on this device.', anyOf: ['borrow_items'] },
      { name: 'Submit borrow request', where: 'Cart', does: 'Creates and submits the request to the depot.', anyOf: ['borrow_items'] },
      { name: 'Browse tools', where: 'Empty cart', does: 'Returns to the catalog.' },
    ],
    tips: [
      { text: 'Cart contents stay in this browser until you submit or empty them.', anyOf: ['borrow_items'] },
      { text: 'Pick-up depot and property must match where you are allowed to work.', anyOf: ['borrow_items'] },
    ],
    troubles: [
      {
        problem: 'Add to cart is missing or disabled',
      ...
      },
      {
        ...
        fix: 'Check that return date is after start date, property and depot are selected, and at least one line remains in the cart.',
        anyOf: ['borrow_items'],
      },
    ],
  },
